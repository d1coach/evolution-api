...
export class GroupRouter extends RouterBroker {
  constructor(...guards: RequestHandler[]) {
    ...
    this.router
      ...
      .get(this.routerPath('fetchSubgroupsCommunity'), ...guards, async (req, res) => {
        const response = await this.groupValidate<GroupJid>({
          request: req,
          schema: groupJidSchema,
          ClassRef: GroupJid,
          execute: (instance, data) => groupController.fetchSubgroupsCommunity(instance, data),
        });
      ...
      .post(this.routerPath('updateJoinRequest'), ...guards, async (req, res) => {
        const response = await this.groupValidate<GroupUpdateJoinRequestDto>({
          request: req,
          schema: updateJoinRequestSchema,
          ClassRef: GroupUpdateJoinRequestDto,
          execute: (instance, data) => groupController.updateJoinRequest(instance, data),
        });
